...
export function SourceSelector() {
	...
	const [maskShape, setMaskShape] = useState<WebcamMaskShape>(() => {
		// Load saved preference
		const prefs = loadUserPreferences();
		return prefs.webcamMaskShape;
	});

	// Save mask shape when it changes
	const handleMaskShapeChange = (shape: WebcamMaskShape) => {
		setMaskShape(shape);
		saveUserPreferences({ webcamMaskShape: shape });
	};
		...
		<div className={`min-h-screen flex flex-col ${styles.glassContainer}`}>
			<div className="flex-1 flex flex-col w-full px-4 pt-4">
				...
				<div className="mt-4 mb-2">
					<p className="text-xs text-zinc-400 mb-2">{t("cameraPreview.title")}</p>
					<CameraPreview maskShape={maskShape} onMaskShapeChange={handleMaskShapeChange} />
				</div>
			</div>
